...
router.delete('/:idTraitement', (req, res) => {
    ...
    models.Traitement.destroy({
        where: { id: idTraitementParam }
    }).then(function(isDeleted) {
        if(isDeleted == true) {
            return res.status(200).json(isDeleted);
        } else {
            return res.status(400).json({ 'Error': ' Le traitement n\'a pas été supprimé' });
        }
    }).catch(function(error) {
        return res.status(500).json({ 'Error ': error + ' Suppression du traitment impossible' });
    });
});
